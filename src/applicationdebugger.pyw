...
def flatten_text(obj) -> str:
    ...
    if isinstance(obj, list):
        return "\n".join(flatten_text(item).strip() for item in obj if flatten_text(item).strip())
    if isinstance(obj, dict):
        for key in ("answer", "final_answer", "solution", "result", "output_text", "text", 
                   "content", "message", "response", "detail"):
            if key in obj:
                text = flatten_text(obj.get(key)).strip()
                if text:
                    return text
        if "choices" in obj:
            return flatten_text(obj["choices"])
        if "output" in obj:
            return flatten_text(obj["output"])
        parts = [flatten_text(v) for v in obj.values() if flatten_text(v).strip()]
        return "\n".join(parts)
    return str(obj)
...
def strip_heading_noise(text: str) -> str:
    ...
    noisy = ("aufgabe:", "frage:", "titel:", "task:", "question:", "title:", 
             "hier ist die aufgabe", "die aufgabe lautet", "lösung:", "answer:", 
             "die antwort ist", "die lösung ist", "final answer")
    
    while len(lines) > 1:
        first = lines[0].lower()
        if any(first.startswith(n) for n in noisy) or (len(lines[0]) < 70 and not re.search(r"[\.\?\!:=]$", lines[0])):
            lines.pop(0)
            continue
        break
    return "\n".join(lines).strip()
...
def normalize_ai_answer(text: str) -> str:
    ...
    parts = []
    for p in s.split("|"):
        p = strip_heading_noise(p)
        p = re.sub(r"^\s*\d+[\.\)]\s*", "", p)
        p = p.strip("-*• ").strip()
        if p:
            parts.append(p)
    
    return " | ".join(parts) if parts else "Keine Antwort"
...
def extract_ki_text(response) -> str:
    ...
    try:
        ...
        return normalize_ai_answer(text or raw)
    except Exception:
...
- Keine Überschriften, keine Erklärungen, kein "Antwort:", "Lösung:" etc.
...
def request_ki_text(question: str, timeout=(5, 120)) -> str:
    ...
    try:
        ...
        if response.status_code >= 400:
            return f"KI HTTP {response.status_code}"
        return extract_ki_text(response) or "Keine Antwort"
    except Exception as e:
...
def request_ki_image(question: str, bbox) -> str:
    ...
    try:
        ...
        payload = {"message": prompt, "image": f"data:image/jpeg;base64,{base64_image}"}

        response = requests.post(f"{KI_SERVER_URL}/vision", json=payload, timeout=(5, 180))
        return extract_ki_text(response) or "Keine Antwort"
    except Exception as e:
        return f"KI Bild-Fehler: {type(e).__name__}"


# ------------------------------------------------------------
# QA (Platzhalter - wird nicht mitgeliefert)
# ------------------------------------------------------------
QA = {
    ...
    "Welches Schutzziel beschreibt, dass Daten vollständig und unverändert beim Empfänger ankommen?": "Integrität",
    "Welche Aussagen beschreiben Authentizität?": "ewsddkadifdmih | dedstvdaq",
    "Welche Aussagen beschreiben Vertraulichkeit bei öffentlicher Kommunikation?": "eikuim | ddkvdgangiw",
    "Ordne die Kontrollmaßnahmen nach Anlage zu § 9 BDSG zu.": "1 > Zutrittskontrolle | 2 > Zugangskontrolle | 3 > Zugriffskontrolle | 4 > Weitergabekontrolle | 5 > Eingabekontrolle | 6 > Auftragskontrolle | 7 > Verfügbarkeitskontrolle | 8 > Trennungsgebot",
    "Welche Kontrollmaßnahme verhindert unbefugten Zutritt zu Datenverarbeitungsanlagen?": "Zutrittskontrolle",
    "Welche Kontrollmaßnahme verhindert die Nutzung von Datenverarbeitungssystemen durch Unbefugte?": "Zugangskontrolle",
    "Welche Kontrollmaßnahme stellt sicher, dass Berechtigte nur auf berechtigte Daten zugreifen können?": "Zugriffskontrolle",
    "Welche Kontrollmaßnahme schützt Daten bei Übertragung, Transport und Speicherung auf Datenträgern?": "Weitergabekontrolle",
    "Welche Kontrollmaßnahme macht nachträglich prüfbar, wer Daten eingegeben, verändert oder entfernt hat?": "Eingabekontrolle",
    "Welche Kontrollmaßnahme stellt Verarbeitung nur nach Weisung des Auftraggebers sicher?": "Auftragskontrolle",
    "Welche Kontrollmaßnahme schützt personenbezogene Daten gegen zufällige Zerstörung oder Verlust?": "Verfügbarkeitskontrolle",
    "Welche Kontrollmaßnahme stellt getrennte Verarbeitung für unterschiedliche Zwecke sicher?": "Trennungsgebot",
    "Welche Fachbegriffe passen zu Datenübertragung und Eingabeprüfung?": "Datenübertragung/Transport/Speicherung > Weitergabekontrolle | nachträgliche Prüfung von Eingabe/Änderung/Entfernung > Eingabekontrolle",
    "Welche Aussagen zum IT-Grundschutz-Check sind zutreffend?": "eiedbdevsze | eiieszsutus",
    "Welche Verfahren nutzt man im IT-Grundschutz-Check für eine Client-Gruppe?": "sldvdzkdc | susvowdcks | sfimdzs",
    "Ordne den PDCA-Zyklus zu.": "Plan > Planung von Sicherheitsmaßnahmen | Do > Umsetzung der Maßnahmen | Check > Erfolgskontrolle, Überwachung der Zielerreichung | Act > Beseitigung von Defiziten, Verbesserung",
    "Welche Aufgaben hat ein Informationssicherheitsbeauftragter?": "dszsuzk | dlbdedszu | dedsuzturzk | rfsasiuziuzü | dluasüdsdizb | sicherheitsrelevante Projekte zu koordinieren | svzussusziziuzk",
    "Ordne die Schutzbedarfskategorien zu.": "niedrig bis mittel > Schäden begrenzt und überschaubar | hoch > Schäden können beträchtlich sein | sehr hoch > Schäden können existenziell bedrohlich sein",
    "Schutzbedarf: Prüfzifferverfahren bei Übermittlung einer Krankenversicherungsnummer.": "Schutzziel > Integrität | Kategorie > hoch | Begründung > verfälschte Daten führen zu fehlerhaften Rechnungen, Behandlungen",
    "Schutzbedarf: Textverarbeitung.": "Schutzziel > Verfügbarkeit | Kategorie > mittel | Begründung > Briefe, Rechnungen verzögern sich",
    "Schutzbedarf: telemedizinische Beratung via Videokonferenz.": "Schutzziel > Vertraulichkeit | Kategorie > hoch | Begründung > Die Privatsphäre wird verletzt",
    "Schutzbedarf: Patientenakten digital verarbeiten.": "Schutzziel > Integrität | Kategorie > sehr hoch | Begründung > falsche Daten führen zu fehlerhaften Behandlungen",
    "Welche Maßnahmen gehören zum Schutzziel Vertraulichkeit?": "sichere Passwörter wählen | Verschlüsselung von Festplatten | ddpvuzs",
    "Welche Maßnahmen gehören nicht direkt zum Schutzziel Vertraulichkeit?": "regelmäßige Sicherung sensibler Personendaten | zsbvd",
    "Welche Aussagen beschreiben die Zweckbindung nach Art. 5 DSGVO?": "pddnffeulzew | ewfiöilafwohfofszguvavmduz",
    "Ordne Sachverhalte Datenschutz oder Datensicherheit zu.": "Kundendaten an Arbeitgeber weitergeleitet > Datenschutz | Buchungen durch technischen Defekt verloren > Datensicherheit | Server mit technischen Daten durch Stromausfall ausgefallen > Datensicherheit | Kundendaten an Fitness-Food-Hersteller für Werbung > Datenschutz | unberechtigte Person speichert Kunden- und Firmendaten auf Stick > Datenschutz und Datensicherheit | Videoüberwachung in Geschäftsräumen > Datenschutz | Gesundheitsforschung mit KI an Universität > Datenschutz | Sicherheitscode beschafft um Kontaktdaten zu bekommen > Datenschutz und Datensicherheit | fremde Person verschafft Zutritt zum Serverraum > Datenschutz und Datensicherheit",
    "Welche Aussagen beschreiben die Speicherbegrenzung nach Art. 5 DSGVO?": "sinslewezbeb | lskfaiöizs",
    "Fall 1 DSGVO: Name und E-Mail-Adresse für Angebotsanfrage speichern erlaubt?": "Ja | zdevmn",
    "Fall 2 DSGVO: Kundendaten mit Postadresse und Social-Media-Daten zu Profil ergänzen erlaubt?": "Nein | epdigv | epdbee",
    "Fall 3 DSGVO: Angebot per E-Mail an gespeicherte E-Mail-Adresse senden erlaubt?": "Ja | edfekmdke | Erlaubt, da für die Angebotsabgabe erforderlich",
    "Was ist Datenschutz?": "spdvmv",
    "Welche Elemente umfasst Datenschutz?": "Schutz des Persönlichkeitsrechts | Schutz der Privatsphäre",
    "Was ist Datensicherheit?": "dbsmdgsvd",
    "Welche Elemente umfasst Datensicherheit?": "Schutz vor Verlust | Schutz vor Verfälschung",
    "Risiko: Mitarbeiter verändert Rechnungsdatum mehrerer bezahlter Kundenrechnungen.": "Bezeichnung > Datenmanipulation | Abwehr > User der letzten Änderung der Datensätze in einer Log-Datei festhalten | Abwehr > Plausibilitätsprüfungen gegen andere Abrechnungssysteme | Abwehr > Änderungen der Datensätze mit einem Zeitstempel nachvollziehbar machen",
    "Risiko: fremde Person nutzt freien PC-Arbeitsplatz in der Verkaufsabteilung.": "Bezeichnung > unberechtigter Zugriff | Abwehr > Zutrittskontrolle | Abwehr > Zugangskontrolle | Abwehr > Zugriffskontrolle",
...
# ------------------------------------------------------------
FONT_NAME = "Arial"
FONT_SIZE = int(cfg.get("FONT_SIZE", 7))
...
status_win.geometry(f"5x2+{status_win.winfo_screenwidth()-5}+{status_win.winfo_screenheight()-2}")
...
def set_processing(on: bool):
    set_status(DARKMAGENTA if on else (GREEN if globals().get('listening', False) else ORANGE))
...
def run_worker(work_fn, done_fn=None, fallback_error="Fehler"):
    def target():
        try:
            ...
            result = f"{fallback_error}: {type(e).__name__}"
        if done_fn:
...
def start_mouse_capture_and_ocr_async():
    ...
    def work():
        ...
        return request_ki_image("Löse die Aufgabe im Screenshot.", (left, top, right, bottom))
...
def show_answer(answers):
    global current_answers, current_answer_index, current_variants, current_variant_index
    try:
        if isinstance(answers, str):
            current_answers = [normalize_ai_answer(answers)]
        elif isinstance(answers, list):
            current_answers = [normalize_ai_answer(a) for a in answers if a]
        else:
            current_answers = ["Keine Antwort"]

        if not current_answers:
            current_answers = ["Keine Antwort"]

        current_answer_index = 0
        current_variants = [normalize_ai_answer(a) for a in current_answers[0].split("|") if a]
        current_variant_index = 0
        update_label_with_current_variant()
        force_show_overlay()
    except Exception:
        label.configure(text="Anzeigefehler", fg=RED)
...
def update_label_with_current_variant():
    if not current_answers:
        ...
        return
    txt = current_variants[current_variant_index] if current_variants else "Keine Antwort"
    label.configure(text=txt, fg=TEXT_COLOR)
...
def set_answer_bindings():
    # Vereinfachte Bindings (du kannst sie erweitern)
    root.bind_all("<Button-2>", lambda e: next_answer())
    root.bind_all("<Button-3>", lambda e: next_answer())
    root.bind_all("<Return>", lambda e: next_answer())
...
def next_answer(event=None):
    ...
    if current_answers:
        ...
        # Varianten neu laden (vereinfacht)
        show_answer(current_answers)
    return "break"
...
def on_status_click(event=None):
    ...
    if listening:
        ...
        listening = True
        set_status(GREEN)
        # Hier könntest du das Such-Overlay starten
    return "break"
